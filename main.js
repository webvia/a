...
var e_gscript = document.querySelector('div[data-code*="data-site"]');
if ( HasObj(e_gscript) ) { var e_gsite = e_gscript.dataset['code'].match(/data\-site\=\".+?\"/g)[0].split('"')[1]; };
cl(e_gsite);
...
xrtu_a.push(url.split('#')[0]); xrt_a.push(xrt); if (type === 'pages') { Pages() } else if (type === 'contents') { Contents() } else if (type === 'cont') { Conts('cont') } else if (type === 'rep') { Reps('rep') } else if (type === 'item') { Item('item') } } } } // }else
}
// DataParse ----------------------------------------------------------------------------------------------------------
function DataParse() {
